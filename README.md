...
此仓库是由 vue3 + ts + vite2 + element-plus 搭建的博客项目。
...
在开发一个组件的过程中，data、computed、methods 这类选项式 API 将会随着业务逻辑的更迭而变得愈发的庞大和复杂，同一个逻辑关注点可能分散在不同的选项中，这会使得后续的维护变得困难，因此我们需要通过组合式 API 将同一逻辑点的东西放在一起复用以此来尽量规避这种情况。
